...
// Smock Added Start
template <class T, int dim>
void Read_ConstraintInfo(const std::string& filePath, 
    std::vector<T>& stitch_rstLen, std::vector<VECTOR<T, 3>>& target_pos, std::vector<int>& target_pos_idx)
{
    std::ifstream is(filePath);
    if (!is.is_open()) {
        puts((filePath + " not found!").c_str());
    ...
    while(std::getline(is, line)){
        ...
        if (line[0] == 's' && line[1] == ' ') {
            ss.ignore(128, ' ');
            ss.ignore(128, ' ');    // discard stitch idx since with order

            stitch_rstLen.resize(stitch_rstLen.size() + 1);
            ss >> stitch_rstLen.back();            
        }
        else if (line[0] == 'v' && line[1] == ' ') {
            ss.ignore();
            target_pos_idx.resize(target_pos_idx.size() + 1);
            ss >> target_pos_idx.back();

            target_pos.resize(target_pos.size() + 1);
            ss >> target_pos.back()[0] >> target_pos.back()[1] >> target_pos.back()[2];
        }
    }

    // // test output
    // for (int i = 0; i < stitch_rstLen.size(); i++) {
    //     // std::cout << 'rstLen: ' << stitch_rstLen[i] << std::endl;
    //     printf("rstLen: %le\n", stitch_rstLen[i]);
    // }

    // for (int i = 0; i < target_pos.size(); i++) {
    //     // std::cout << 'target idx: ' << target_pos_idx[i]
    //     //     << ' pos: ' << target_pos[i][0] << ' ' << target_pos[i][1] << ' ' << target_pos[i][2] << std::endl;
    //     printf("target idx: %d pos: %le %le %le\n", target_pos_idx[i], target_pos[i][0], target_pos[i][1], target_pos[i][2]);
    // }
...
void Export_MeshIO(py::module& m) {
    ...
    // Smock Added Start
    m.def("Read_ConstraintInfo", &Read_ConstraintInfo<double,3>, "read constraintInfo file");
    // Smock Added End
