...
bool Compute_IncPotential(
    ...
    T weight_tgt_pos = 0.0,
    const std::vector<VECTOR<T, 3>>& target_pos = std::vector<VECTOR<T, 3>>(),
    const std::vector<int>& target_pos_idx = std::vector<int>()
    // Smock Added End
    )
{
    ...
    if constexpr (flow) {
        ...
        // garment
        Compute_Stitch_Energy(X, stitchInfo, stitchRatio, DBCb, k_stitch, h, value, stitch_rstLen); // Smock Added Line (1 param)

        // Smock Added Start
        // Add positional constraint TODO: try 2D and 3D?
        for (int i = 0; i < target_pos_idx.size(); ++i){
            int x_idx = target_pos_idx[i];
            const VECTOR<T, dim>& x_cur = std::get<0>(X.Get_Unchecked(x_idx));
            const VECTOR<T, dim> x_tgt = target_pos[i];
            // printf("energy: %le \n", (x_cur - x_tgt).length2());
            // printf("pos cur: %le %le %le\n", x_cur[0], x_cur[1], x_cur[2]);
            // printf("pos tgt: %le %le %le\n", x_tgt[0], x_tgt[1], x_tgt[2]);
            value += weight_tgt_pos * (x_cur - x_tgt).length2();        // 3-dim
            // value += weight_tgt_pos * ((x_cur[0] - x_tgt[0])*(x_cur[0] - x_tgt[0]) + (x_cur[2] - x_tgt[2])*(x_cur[2] - x_tgt[2]));  // 2-dim
        }
        // Smock Added End
    ...
    )
{
    ...
    if constexpr (flow) {
        ...
        // garment
        Compute_Stitch_Gradient(X, stitchInfo, stitchRatio, DBCb, k_stitch, h, nodeAttr, stitch_rstLen); // Smock Added Line (1 param)

        // Smock Added Start
        // Add positional constraint TODO: try 2D and 3D?
        T w_tgt_pos = 2.0 * weight_tgt_pos;
        for (int i = 0; i < target_pos_idx.size(); ++i){
            int x_idx = target_pos_idx[i];
            const VECTOR<T, dim>& x_cur = std::get<0>(X.Get_Unchecked(x_idx));
            const VECTOR<T, dim> x_tgt = target_pos[i];

            VECTOR<T, dim>& grad_x = std::get<FIELDS<MESH_NODE_ATTR<T, dim>>::g>(nodeAttr.Get_Unchecked(x_idx));
            grad_x += w_tgt_pos * (x_cur - x_tgt);      // 3-dim

            // // 2-dim
            // grad_x[0] += w_tgt_pos * (x_cur[0] - x_tgt[0]);
            // grad_x[2] += w_tgt_pos * (x_cur[2] - x_tgt[2]);
        }
        // Smock Added End
    ...
    )
{
    ...
    if constexpr (flow) {
        ...
        // garment
        Compute_Stitch_Hessian(X, stitchInfo, stitchRatio, DBCb, k_stitch, h, projectSPD, triplets, stitch_rstLen); // Smock Added Line (1 param)

        // Smock Added Start
        // Add positional constraint TODO: try 2D and 3D?
        int startIdx_tri = triplets.size();

        // 3-dim
        triplets.resize(triplets.size() + 3 * target_pos_idx.size());       // each pos constraint induce 3-dim hessian for that vert
        T w_tgt_pos = 2.0 * weight_tgt_pos;
        for (int i = 0; i < target_pos_idx.size(); ++i){
            int x_dim_idx = target_pos_idx[i] * 3;      // x,y,z

            for (int d = 0; d < 3; ++d){
                triplets[startIdx_tri + i * 3 + d] = Eigen::Triplet<T>(
                    x_dim_idx + d, x_dim_idx + d, w_tgt_pos);
            }
        }

        // // 2-dim
        // triplets.resize(triplets.size() + 2 * target_pos_idx.size());       // each pos constraint induce 3-dim hessian for that vert
        // T w_tgt_pos = 2.0 * weight_tgt_pos;
        // for (int i = 0; i < target_pos_idx.size(); ++i){
        //     int x_dim_idx = target_pos_idx[i] * 3;      // x,y,z

        //     triplets[startIdx_tri + i * 3] = Eigen::Triplet<T>(
        //         x_dim_idx, x_dim_idx, w_tgt_pos);
        //     triplets[startIdx_tri + i * 3 + 2] = Eigen::Triplet<T>(
        //         x_dim_idx + 2, x_dim_idx + 2, w_tgt_pos);
        // }
