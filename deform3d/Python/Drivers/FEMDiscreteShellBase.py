...
class FEMDiscreteShellBase(SimulationBase):
    def __init__(self, precision, dim):
        ...
        ## Smock Added Start
        self.stitch_rstLen = StdVectorXd()      # vector 1-1 correspondence (per stitching line)

        self.weight_tgt_pos = 0.0               # weight for constraint energy
        self.target_pos = StdVectorVector3d()      
        self.target_pos_idx = StdVectorXi()
        ## Smock Added End
    ...
    def add_garment_3D(self, filePath, translate, scale, rotCenter, rotAxis, rotDeg):
        ...
        #MeshIO.Write_TriMesh_Obj(self.X_stage, self.Elem, self.output_folder + "test" + ".obj")
    ...
    ## Smock Added Start
    def add_constraint_file(self, filepath, weight_pos):
        '''
        read .constraintInfo file using added MESHIO func
        output param:
            self.stitch_rstLen = StdVectorXd()      # vector 1-1 correspondence (per stitching line)
            self.weight_tgt_pos = 0.0               # weight for constraint energy
            self.target_pos = Storage.V3dStorage()       
            self.target_pos_idx = Storage.V3iStorage()
        '''
        self.weight_tgt_pos = weight_pos
        MeshIO.Read_ConstraintInfo(filepath, self.stitch_rstLen, self.target_pos, self.target_pos_idx) # MESHIO func to assign value ()
    ## Smock Added End
            ...
            if self.flow:
                ...
                if self.split:
                    ...
                    self.PNIterCount = self.PNIterCount + FEM.DiscreteShell.Advance_One_Step_IE_Hinge(self.Elem, self.segs, self.DBC, \
                        ...
                        self.tet, self.tetAttr, self.tetElasticity, self.rod, self.rodInfo, \
                        self.rodHinge, self.rodHingeInfo, self.stitchInfo, self.stitchRatio, self.k_stitch,\
                        self.discrete_particle, self.output_folder, self.stitch_rstLen, self.weight_tgt_pos, self.target_pos, self.target_pos_idx)
                    ## Smock Added End
        self.t += dt
